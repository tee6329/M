...
def listgrup():
    listGrup =  """💢คำสั่งตั้งค่า ─┅═ই☣さণさபิວ☣ই═┅─💢
...
def helpkicker():
    helpKicker = """⏳คำสั่งค้นหา Bill&BotSelf⏳
...
def helpsetting():
    helpSetting = """⚓คำสั่งกลุ่ม Bill&BotSelf⚓
🇵🇳 กลุ่ม ➠ ข้อมูลกลุ่ม
🇵🇳 ไอดีกลุ่ม ➠ไอดีห้อง
🇵🇳 ชื่อกลุ่ม ➠ ชื่อห้อง
...
def helptexttospeech():
    helpTextToSpeech =   """💠คำสั่งเลียนแบบbill&BotSelf💠
🌏 เลียนแบบ on/off 
🌏 เลียนแบบ 
🌏 เพิ่มเลียนแบบ (@) 
🌏 ลบเลียนแบบ (@)"""
...
def helplanguange():
    helpLanguange =    """💨คำสั่งล่องหนbill&BOT💨
⚓!แทค
⚓!มิด
⚓!คท"""
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    if msg.toType == 2:
                        ...
                        try:
                            ...
                            pass
                elif text.lower() == 'จรวจ':
                    start = time.time()
                    line.sendMessage(to,"ความเร็วของ ☣さণさபิວ☣ Self...")
...
#=============COMMAND KICKER===========================#
                    ...
                    settings["blacklist"] = {}
                    line.sendMessage(msg.to,"ทำการลบหำทั้งหมดเรียบร้อย")
                    print ("Clear Ban")

                elif 'อีดอก' in text.lower():
                       targets = []
                       key = eval(msg.contentMetadata["MENTION"])
                       key["MENTIONEES"] [0] ["M"]
                       for x in key["MENTIONEES"]:
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in bot1:
                        if settings["blacklist"] == {}:
                            line.sendMessage(msg.to,"ไม่พบหำในห้องนี้")
                        else:
                            line.sendMessage(msg.to,"โปรดรอ...")
                            num=1
                            msgs="══════════รายชื่อผู้ที่หำเหม็น═════════"
                      ...
                      if msg.toType == 2:
                       ...
                       for target in targets:
                           try:
                               ...
                               f=codecs.open('st2__b.json','w','utf-8')
                               json.dump(settings["blacklist"], f, sort_keys=True, indent=4,ensure_ascii=False)
                               line.sendMessage(msg.to,"แก้หำเหม็นเรียบร้อย...")
                               print ("Unbanned User")
                           except:
                               line.sendMessage(msg.to,"ไม่พบผู้ติดต่อ")
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in bot1:
                        if settings["blacklist"] == {}:
                            ...
                            line.sendMessage(msg.to,"รายชื่อผู้ที่หำเหม็น")
                            num=1
