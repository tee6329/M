...
settings = {
    ...
    "man2":"ตั้งข้อความ",
    "man3":"ตั้งข้อความ",
    "message":"บัญชีนี้ได้การป้องกัน💢โดย เอ็ม Bot Self ระบบได้ทำการบล็อคคุณอัตโนมัติ\nติดต่อเอ็มได้ที่ Em_club0075",
...
dangerMessage2 = ["ควย","หี","แตด","เย็ดแม่","เย็ดเข้","ค.วย","สัส","เหี้ย","ไอ้เหี้ย","พ่อมึงตาย","ไอ้เลว","ระยำ","ชาติหมา","หน้าหี","เซลกาก","พ่องตาย","ส้นตีน","แม่มึงอ่ะ","แม่มึงดิ","พ่อมึงดิ"]
...
def listgrup():
    listGrup =  """💢คำสั่งตั้งค่า เอ็มBotSelf💢
...
🌠เปิดอ่าน ➠เปิดจับคนแอบอ่าน
🌠ปิดอ่าน ➠ปิดจับคนแอบอ่าน
...
def helpkicker():
    helpKicker = """⏳คำสั่งค้นหา เอ็มBotSelf⏳
...
def helpsetting():
    helpSetting = """⚓คำสั่งกลุ่ม เอ็มBotSelf⚓
🍁 กลุ่ม ➠ ข้อมูลกลุ่ม
🍁 ไอดีกลุ่ม ➠ไอดีห้อง
🍁 ชื่อกลุ่ม ➠ ชื่อห้อง
...
def helptexttospeech():
    helpTextToSpeech =   """⚡คำสั่งเลียนแบบเอ็มBotSelf⚡
🔁 เลียนแบบ on/off 
🔁 เลียนแบบ 
🔁 เพิ่มเลียนแบบ (@) 
🔁 ลบเลียนแบบ (@)"""
                         ...
                         if targets == []:
                             ...
                             for target in targets:
                                 try:
                                     ...
                                     line.sendText(msg.to,"ทำการเชิญคนนี้สำเร็จ \n➠" + _name)
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    ki.sendMessage(to, "เอ็ม BotSelfยังอยู่ครับ")
                elif text.lower() == 'เทส':
                    line.sendMessage(to, "เอ็ม BotSelfกำลังโหลด:▒...0%")
                    ...
                    line.sendMessage(to, " 🔱เอ็ม BotSelf ยังอยู่ครับ✔")
                    ...
                    start = time.time()
                    line.sendMessage(to,"ความเร็วของ เอ็ม Bot Self...")
                    ...
                    if LINE != None:
                         ...
                         if spl[0] == "":
                            ...
                            if msg.toType == 2:
                                ...
                                num = int(strnum)
                                line.sendMessage(to, "เชิญเข้าร่วมการโทรเรียบร้อยแล้วครับ")
...
#==============================================================================#   
                    ...
                    with requests.session() as web:
                        ...
                        ret_ = "╔══[ ผลการค้นหาของ เอ็มBotSelf ]"
                        datas = []
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["speed","sp","Speed","สปีด","!speed","/speed"]:
                    line.sendText(msg.to,"แรงขนาดนี้ เอ็มนี้ไลค์เลย👍")
                ...
                if msg.text in ["555","5555","55555","55+","55++","555+","555++","5555+","5555++"]:
                    line.sendText(msg.to,"เอ็มว่าอาการหนักนะเนี๊ยะ!!. อิอิ😝😝")				
                if msg.text in dangerMessage:
                    random.choice(Rfu).kickoutFromGroup(receiver,[sender])
                    random.choice(Rfu).sendText(msg.to,"ตรวจพบคำสั่งของบอทลบกลุ่ม  เอ็ม BotSelf จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก")										
        if op.type == 17:
          if settings["Wc"] == True:
                ...
                if RfuCctv['cyduk'][op.param1]==True:
                    if op.param1 in RfuCctv['point']:
                        ...
                        if Name in RfuCctv['sidermem'][op.param1]:
                            ...
                            pref=['สวัสดีคนอ่าน➠','ตร๊ะ!..เอ๋!!..😝➠','หวัดดีจร้า💖➠','เห็นนะ อิอิ..😝➠','แอบทำมัย? มาคุยกันหน่อย➠','แอบแฟนเล่นหราา💨➠','รู้นะว่าคัยอ่าน👉➠','เล่นซ่อนแอบหรอคร้าบบ👻➠','มาคุยกับเค้าหน่อย➠','นั่นแน่ แอบหราาา➠','จร๊ะ!!..เอ๋!..➠']
